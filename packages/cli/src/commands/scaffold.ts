...
import eleventyConfigTemplate from '../templates/eleventy-config.js';
...
const writeScripts = async () => {
    ...
    await Promise.all(Object.entries(scripts).map(([scriptName, { script, description }]) => {
...
const scaffoldFastify = async ({ projectName, options }: { projectName: string, options: ScaffoldCommandOptions }) => {
    dependencies.push('@fastify/autoload', '@fastify/sensible', '@fastify/static','fastify', 'fastify-cli', 'fastify-plugin');
    devDependencies.push('@11ty/eleventy');
